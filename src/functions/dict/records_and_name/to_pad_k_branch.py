...
def f(x):
  q = [
    f_b({'records': x['records'], 'k_branch': x['name'], 'k_leaf': k_leaf})
    for k_leaf
    in f_c(x)
  ]
  w = abs(f_a(q))
...
t_prices = lambda: pxyf(
  {'records': records_without_date, 'name': 'prices'},
  '            prices',
  f
)

t_volumes = lambda: pxyf(
  {'records': records_without_date, 'name': 'volumes'},
  '                          volumes',
  f
)

t_zloops = lambda: pxyf(
  {'records': records_without_date, 'name': 'zloops'},
  'zloops',
  f
)

t_date = lambda: pxyf(
  {'records': records_with_date, 'name': 'date'},
  10,
  f
)
